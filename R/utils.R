...
findTicks <- function(r, tick_count, preferred_gap_sizes, preserve_sign = FALSE){
  ...
  lb[lb_too_low] <- lb[lb_too_low] + floor(gaps[lb_too_low]/2)
  ub[lb_too_low] <- ub[lb_too_low] + floor(gaps[lb_too_low]/2)
  
  ub_too_high <- r[2] < ub - gaps & (!preserve_sign | sign(ub) == sign(ub - gaps/2))
  lb[ub_too_high] <- lb[ub_too_high] - ceiling(gaps[ub_too_high]/2)
  ub[ub_too_high] <- ub[ub_too_high] - ceiling(gaps[ub_too_high]/2)
