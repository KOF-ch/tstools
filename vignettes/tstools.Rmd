---
title: "tstools - A Time Series Toolbox for Official Statistics"
...
objects are passed on to the function the method dispatcher choose the right method automatically and plots the graph. The following sections will walk through several applied plotting examples. Horizontal grids, automatic shifting of series to the middle of the period, colors, line types, filling up started years and many more come as convenient defaults but can be adjusted using *themes*. 
...
quarters are denoted by small tick marks, years are denoted by longer ticks. Note, the original time series
is just shortened a bit to zoom in a bit. The object *short* is of class ts.

```{r,fig.width = 7,fig.height=6,message=FALSE}
library(tstools)
data(KOF)
short <- window(KOF$kofbarometer,start=c(2007,1),end=c(2014,1))
tsplot(short,manual_value_ticks_l = seq(60,130,by=10))
```
...
tsplot(ts1,ts2,manual_value_ticks_l = seq(-10,60,by=10))
```
or a list of time series:
```{r,fig.width = 7,fig.height=6}
tsplot(tslist,manual_value_ticks_l = seq(-10,60,by=10))
```
...
tsplot(KOF$kofbarometer,
       tsr = KOF$reference,
       manual_value_ticks_l = seq(50,130,by=10),
       manual_value_ticks_r = seq(-8,8,by=2))
```
