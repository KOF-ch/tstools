...
show examples of the most interesting ones. You may also want to look at `?initDefaultTheme`
or run the following to see all available options.

```{r}
def_theme <- initDefaultTheme()
names(def_theme)
```
...
```{r,eval=FALSE}
data(KOF)
writeTimeSeries(KOF,"test_export","csv")
```

```{r,eval=FALSE}
writeTimeSeries(KOF,"test_export_wide_trans","csv", wide = T, transpose = T)
```
