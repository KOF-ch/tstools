...
### Single time series: bar chart
```{r,fig.width = 7}
library(tstools)
data(KOF)
baro_short <- window(KOF$kofbarometer,start=c(2010,1))
tsBarPlot(KOF$kofbarometer,theme = d)
```
...
## Advanced use
This section provides application examples covering the following topics: 
Barcharts, combination of bar and line charts, multiple Y axes and pdf export.
The subsequent *themes* sections discusses manyfold layout options and how themes
can be defined as layout templates for multiple plots. 
