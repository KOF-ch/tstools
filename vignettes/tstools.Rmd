...
With tstools' tsplot function you can assign confidence intervals to every 
time series line. Simply choose the line and confidence level and define an upper
and lower bound ts object to draw shaded confidence bounds around a respective series. 
A confidence interval definition is basically a nested list. For the sake of clear
code, it is recommended to define the CI list separately like so: 
...
The KOF data example dataset contains an auto.arima point forecast of the KOF Barometer
as well as upper and lower bound at the 80% and 95% confidence level. Notice that 
tsplot does no forecasting or estimation of confidence bands, it simply takes time series as upper 
and lower bounds and assign them to a particular series. Thus it's agnostic of the estimation method. 
